...
# Mina Node Monitor
**Mina Node Monitor** это `клиент-серверное` приложение для визуального представления текущего состояния ноды и сигнализации об ошибках в ее работе.

## Основные возможности
1. Отслеживание состояния ноды в реальном времени
...
4. Мониторинг `здоровья` ноды, если нода рассинхронизированны с основной сетью и/или находится в состоянии, отличном от SYNCED, специальный плертер отправляет об этом сообщение вам в телеграм
...
- `Клиент` - используется для визуализации состояния ноды в браузере
- `Сервер` - используется для получения необходимой информации с сервера ноды, отправки ее клиенту и сигнализации о неполадках
...
Секция `hosts` содержит информацию о серверах, на которых установлена серверная чать Монитора. 
Каждый адрес должен определять открытый внешний сетевой интерфейс и его порт.
Параметр `useHost` определяет какой серве из списка в секции `hosts` будет использоваться.
Параметр `showIp` определяет показывать или нет IP адрес в блоке **HOSTNAME**.
...
Каждый запрос выполняется в собственном неблокирующем потоке.

**Интревалы:**

- `info` - основная информация о сервер (ОС, модель процессора)
- `time` - текущее время на сервере и его аптайм
- `blockchain` - всего монет, информация о текущем слоте и эпохе
- `node` - интервал получения информации из GraphQL ноды
...
- `telegramChatIDAlert` - Идентификаторы чатов получателей информации об ошибках ноды (рассинхронизация, выход из статуса SYNCED), можно указать несколько через запятую
...
- `blockDiff` - Разница в высоте блоков с [MinaExplorer](https://minaexplorer.com/) при котром будет считаться, что нода рассихронизированна
- `host` - IP и PORT на котором будет работать сервер монитора
- `graphql` - адрес на котором работает GraphQL сервер ноды
