        ...
        if (blocks) {
            try {
                ...
                if (
                    ...
                    const message = `We got last block at \`${timestamp("-", lastBlockData.dateTime)}\` on height \`${lastBlockData.blockHeight}\` with coinbase \`${lastBlockData.transactions.coinbase / 10 ** 9}\` mina. Total ${blocks.data.blocks.length} blocks.`
